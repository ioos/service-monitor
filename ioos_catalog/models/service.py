...
class Service(BaseDocument):
    ...
    def ping(self, timeout=None):
        ...
        if self.service_type == 'DAP':
            # get a description of the data back from OPeNDAP (this gives
            # proper http codes)
            url += '.das'
        elif self.service_type == 'SOS':
            # modify url to request specific subsection - a ping does not care
            # about data, just that it works
            p = list(urlparse.urlparse(url))
            # parse_qs gives us lists back, we don't want that here
            qdict = dict(urlparse.parse_qsl(p[4]))
    ...
    def count_types_by_provider(cls):
        ...
        # transform into slightly friendlier structure.  could likely do this
        # in mongo but no point
    ...
    def get_failures_in_time_range(self, end_time=None, start_time=None):
        ...
        for p in pls:
            ...
            if sidx < eidx:
                ...
                statuses = p.operational_statuses[
                    sidx:168] + p.operational_statuses[0:eidx]
            ...
            failed_services[p.service_id] = (
                good, count, p.last_operational_status)

        # retrieve all services
        services = list(db.Service.find({'_id': {'$in': failed_services.keys()}}).sort(
            [('data_provider', 1), ('name', 1)]))
