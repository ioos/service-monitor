...
provider_info = {
    ...
    'ATN DAC': {
        ...
        'external_url': 'https://ioos.noaa.gov/news/animal-telemetry-network-atn-dac-launched/'
    },
    'CariCOOS': {
    ...
    'US ACE': {
        ...
        'provider_type': 'national',
        'banner': 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/35/United_States_Army_Corps_of_Engineers_logo.svg/2000px-United_States_Army_Corps_of_Engineers_logo.svg.png',
...
def show_ra(provider):
    ...
    providers = [region['title'] for region in region_map]
    if provider not in providers:
        if provider in provider_info and provider_info[provider].get('external_url'):
            return redirect(provider_info[provider].get('external_url'))
        return redirect('/')
